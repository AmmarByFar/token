...
contract BezosBack is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxBuyTranscationAmount = 500000000000 * (10**18);
    uint256 public maxSellTransactionAmount = 100000000000000 * (10**18);
    uint256 public swapTokensAtAmount = 20000000000 * (10**18);
    uint256 public maxWalletToken = 100000000000000 * (10**18); // 1.5% of total supply
    ...
    constructor() ERC20("BezosBack", "BezosBack") {
        //uint256 _dividendRewardsFee = 10;
        uint256 _marketingFee = 12;

        //dividendRewardsFee = _dividendRewardsFee;
        marketingFee = _marketingFee;
        //totalFees = _dividendRewardsFee.add(_marketingFee);
        totalFees = _marketingFee;

    	//dividendTracker = new BezosBackDividendTracker();

    	buyBackWallet = 0xDB929770B6fb09e1841bC04608845d20d2277a6C;
    ...
    function setTokenSwapThreshold(uint256 amount) external onlyOwner {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "BezosBack: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "BezosBack: Marketing divisor must be between 0 (0%) and 100 (100%)");
  	...
  	function prepareForPreSale() external onlyOwner {
        ...
        marketingFee = 0;
        maxBuyTranscationAmount = 500000000000000 * (10**18);
        maxWalletToken = 500000000000000 * (10**18);
    }
    
    function afterPreSale() external onlyOwner {
        //dividendRewardsFee = 10;
        marketingFee = 12;
        maxBuyTranscationAmount = 300000000000 * (10**18);
        maxWalletToken = 1500000000000 * (10**18);
    }
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "BezosBack: A swapping process is currently running, wait till that is complete");
    ...
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        //require(newFee >= 0 && newFee <= 10, "BezosBack: Dividend reward tax must be between 0 and 10");
        marketingFee = newFee;
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BezosBack: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BezosBack: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BezosBack: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BezosBack: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "BezosBack: The liquidity wallet is already this address");
        excludeFromFees(newBuyBackWallet, true);
        buyBackWallet = newBuyBackWallet;
        emit BuyBackWalletUpdated(newBuyBackWallet, buyBackWallet);
    }

    function sendToBuyBackWallet() public onlyOwner {
        uint256 contractTokenBalance = balanceOf(address(this));
        if (!swapping) {
                swapping = true;
                uint256 swapTokens = contractTokenBalance; //.mul(marketingFee).div(totalFees);
                swapTokensForBNB(swapTokens);
                transferToBuyBackWallet(payable(buyBackWallet), address(this).balance);
                swapping = false;
            }  
    }
